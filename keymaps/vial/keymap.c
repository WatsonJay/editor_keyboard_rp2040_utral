...
#define LAYER_LEFT_TOP_POSITION { 16, 0 }
/* luna */
/* settings */
...
void oled_write_layer_pic(uint8_t now_layer) {
    ...
    for(uint8_t now_line = 0; now_line < 4; now_line++) {
        oled_set_cursor(LAYER_LEFT_TOP_POSITION[0], LAYER_LEFT_TOP_POSITION[1] + now_line);
        oled_write_raw_P(layerPic[now_layer][now_line], LAYER_ICON_WIDTH);
    }
}

void oled_write_layer_pic_clear() {
    for(uint8_t now_line = 0; now_line < 4; now_line++) {
        oled_set_cursor(LAYER_LEFT_TOP_POSITION[0], LAYER_LEFT_TOP_POSITION[1] + now_line);
        oled_write("     ", false);
    }
